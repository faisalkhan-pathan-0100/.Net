...
namespace PracticeDay6A
{
    internal class Program
    {
        ...
        static void Main1() // select with function (whole employee)
        {
            AddRecs();
            //var emps = from emp in lstEmp select emp;
            var emps = lstEmp.Select(GetAllEmps); // with normal function
            var emps2 = lstEmp.Select(delegate (Employee emp) // with anonymous method
            {
                return emp;
            });
            var emps3 = lstEmp.Select(emp => emp); // with lambda function
        ...
        static void Main2() // select perticular property of  employee with all possible way
        {
            AddRecs();
            //var emps = from emp in lstEmp select emp.Name;
            //var emps = from emp in lstEmp select emp.EmpNo; // LINQ query
            var emps = lstEmp.Select(GetAllEmpNames); // with normal function
            var emps2 = lstEmp.Select(delegate (Employee emp) // with anonymous method (using delegate)
            {
                return emp.Name;
            });
            var emps3 = lstEmp.Select(emp => emp.Name); // with lambda function
        ...
        static void Main3() // select with more then one property
        {
            AddRecs();
            var emps = from emp in lstEmp select new { emp.Name, emp.EmpNo }; // simple LINQ query with anonymous class object
            var emps2 = lstEmp.Select(delegate (Employee emp) // with anonymous method (using delegate)
            {
                return new { emp.Name, emp.EmpNo };
            });
            var emps3 = lstEmp.Select(emp => new { emp.Name, emp.EmpNo }); // lambda
            foreach (var item in emps)
            {
        ...
        static void Main4a() // where 
        {
            ...
            var emps2 = lstEmp.Where(IsBasicGreaterThan10000); // normal fucntion
            var emps3 = lstEmp.Where(delegate (Employee emp) // anonymous method
            {
                return emp.Basic > 10000;
            });

            var emps4 = lstEmp.Where(emp => emp.Basic > 10000); // lambda
        ...
        static void Main4()
        {
            ...
            //var emps3 = lstEmp.Select(emp => emp.Name).Where(emp => emp.Basic > 10000); //error
            ...
            //var emps4a = lstEmp.Select(emp => emp.Basic).Where(emp => emp > 10000);
        ...
        static void MainDef()
        {
            ...
            foreach (var emp in emps) // got the updated value
            {
                Console.WriteLine(emp.Name + "," + emp.EmpNo);
            }
            Console.ReadLine();
